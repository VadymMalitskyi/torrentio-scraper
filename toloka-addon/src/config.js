...
const schema = commonSchema.extend({
  ...
  MAX_SEARCH_RESULTS_PER_QUERY: positiveInteger(25),
  TOLOKA_REQUEST_CONCURRENCY: positiveInteger(2),
  MOVIE_TOPIC_FETCH_LIMIT: positiveInteger(3),
  SERIES_TOPIC_FETCH_LIMIT: positiveInteger(5),
  MOVIE_TORRENT_DOWNLOAD_LIMIT: positiveInteger(5),
  SERIES_TORRENT_DOWNLOAD_LIMIT: positiveInteger(5),
  MOVIE_RELEASE_LIMIT: positiveInteger(1),
  SERIES_RELEASE_LIMIT: positiveInteger(4),
  MOVIE_CANDIDATE_DELAY_MS: z.coerce.number().int().min(0).default(6000),
...
export function loadConfig(environment = process.env) {
  ...
  return Object.freeze({
    ...
    maxSearchResultsPerQuery: data.MAX_SEARCH_RESULTS_PER_QUERY,
    ...
    movieCandidateDelayMs: data.MOVIE_CANDIDATE_DELAY_MS,
